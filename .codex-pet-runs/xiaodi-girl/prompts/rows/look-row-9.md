Create one horizontal look-direction strip for Codex pet `xiaodi`, atlas row 9.
...
MANDATORY REPAIR — prior blind QA proved that the previous row accidentally pointed every nominal rightward pose toward the viewer's LEFT. Correct this root failure. For slots 1 through 7, the girl's visible nose tip and both irises must sit on the viewer's RIGHT side of her head center, and the nose/profile must physically project toward the canvas's RIGHT edge. Her long back hair and tail cape should trail more toward the viewer's LEFT as she turns right. Copy the direction meaning of the approved `090` anchor (second pose in the four-cardinal strip), never its character-relative side. At `090`, a label-free reviewer must unmistakably say “screen-right”; if the nose points toward the left edge, the entire row is failed. Do not mirror the girl's hair ornament or outfit markings while correcting the head/body turn.
