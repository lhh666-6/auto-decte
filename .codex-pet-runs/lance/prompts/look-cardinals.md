Create one horizontal four-cardinal anchor strip for Codex pet `lance`.
...
REPAIR REQUIREMENT: the previous strip passed all four direction semantics but failed source-slot edge geometry because fins and tails came too close to neighboring quarters. Preserve those approved up/right/down/left pose meanings, but draw every mascot about 15% smaller and center it in its own quarter with a wide empty green gutter on both sides. No fin, tail, hand, or body pixel may approach or cross an invisible quarter boundary; the four silhouettes must be clearly isolated components.
