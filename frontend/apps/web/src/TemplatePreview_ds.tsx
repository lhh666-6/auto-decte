...
export function TemplatePreview({ api, versionId, onBack, onTune }: Props) {
  ...
  return <main className="template-center template-preview-page">
    <header className="template-center-header"><div><button className="text-button" onClick={onBack}>返回模板库</button><span className="eyebrow">模板版本 · 只读预览</span><h1>{version?.display_name ?? "模板预览"}</h1><p>{version ? `${version.template_key} · ${version.description || "尚未填写用途说明"}` : "已发布内容不可直接修改。"}</p></div>{version && <span className={`status-pill ${version.status === "PUBLISHED" ? "success" : "warning"}`}>{version.status === "PUBLISHED" ? "已发布" : "已退役"} V{version.version}</span>}</header>
