...
export function TemplateStudio({ onBack }: Props) {
  ...
  const [field, setField] = useState({ field_key: "worker_name", display_name: "姓名", data_type: "text", input_type: "text_box", x: 0.1, y: 0.2, width: 0.22, height: 0.05 });
  ...
  return <main className="template-studio">
    ...
    <section className="studio-canvas"><div className="paper-preview"><b>{version?.template_key ?? key} · {page}</b><span className="marker top-left">10</span><span className="marker top-right">11</span><span className="marker bottom-left">13</span><span className="marker bottom-right">12</span><div className="safe-zone">模板 QR 安全区</div>{version?.fields.map((item) => <div key={item} className="template-field">{item}</div>)}</div></section>
    <aside className="studio-card"><h2>字段配置</h2><label>字段键<input value={field.field_key} onChange={(event) => setField({ ...field, field_key: event.target.value })} /></label><label>显示名<input value={field.display_name} onChange={(event) => setField({ ...field, display_name: event.target.value })} /></label><div className="coordinate-grid">{(["x", "y", "width", "height"] as const).map((name) => <label key={name}>{name}<input type="number" step="0.01" min="0" max="1" value={field[name]} onChange={(event) => setField({ ...field, [name]: Number(event.target.value) })} /></label>)}</div><button className="button button-secondary" disabled={!version || version.status === "PUBLISHED"} onClick={() => void addField()}>加入字段</button><button className="button button-secondary" disabled={!version || version.status === "PUBLISHED"} onClick={() => void preflight()}>运行发布预检</button><button className="button button-primary" disabled={!version || version.status !== "READY_TO_PUBLISH"} onClick={() => void publish()}>发布模板</button></aside></section>
