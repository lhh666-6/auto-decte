...
模板中心计划 Task 1–7、四个企业模板的幂等安装、审核工作台事务闭环，以及员工/工单/产品/工序主数据 CRUD 均已完成。下一阶段从模板化导出与重导中心开始。
...
- 员工、工单、产品、工序主数据持久化 CRUD、搜索、版本冲突控制、停用/恢复与审计轨迹
- 模板字段可绑定有效主数据选项；审核前端提供下拉选择，后端再次阻止未知或已停用编码

尚未完成：审核工作台上一张/下一张与高级图像工具、完整规则结果面板、产品化导出中心、模板包/纸张实例闭环、持久化任务 Handler 和 Tauri 桌面壳。主数据和已发布模板均不物理删除；停用/退役后仍保留历史追溯。完整边界以交接文档为准。
