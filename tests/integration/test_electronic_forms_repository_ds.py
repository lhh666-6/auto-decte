...
class TestDefinitionRepository:
    ...
    def test_get_published_returns_latest(
        self, def_repo: SqlAlchemyElectronicFormDefinitionRepository
    ) -> None:
    ...
    def test_get_published_ignores_draft(
        self, def_repo: SqlAlchemyElectronicFormDefinitionRepository
    ) -> None:
        v1 = ElectronicFormDefinitionVersion(
            definition_version_id="efd-draft",
            form_type="FT2",
            version=1,
            status=DefinitionStatus.DRAFT,
            display_name="草稿",
    ...
    def test_unique_form_type_version(
        self, def_repo: SqlAlchemyElectronicFormDefinitionRepository
    ) -> None:
        v1 = ElectronicFormDefinitionVersion(
            definition_version_id="efd-u1",
            form_type="FT3",
            version=1,
            status=DefinitionStatus.DRAFT,
            display_name="v1",
    ...
    def test_presentation_config_round_trip(
        self, def_repo: SqlAlchemyElectronicFormDefinitionRepository
    ) -> None:
        config = PresentationConfig(
            fields=[
                PresentationField(
                    field_key="block_count",
                    display_order=1,
                    strategy="MANUAL_REQUIRED",
                    group="主信息",
                ),
                PresentationField(
                    field_key="total_piece_count",
                    display_order=2,
        ...
        d = ElectronicFormDefinitionVersion(
            definition_version_id="efd-config",
            form_type="FT4",
            version=1,
            status=DefinitionStatus.DRAFT,
            display_name="带配置",
...
class TestDraftRepository:
    ...
    def test_update_increments_revision(
        self, draft_repo: SqlAlchemyElectronicDraftRepository
    ) -> None:
        d = ElectronicDraft(
            draft_id="draft-002",
            owner_actor_id="a1",
    ...
    def test_list_by_owner_filters_correctly(
        self, draft_repo: SqlAlchemyElectronicDraftRepository
    ) -> None:
...
class TestReceiptRepository:
    def test_add_and_retrieve(
        self, receipt_repo: SqlAlchemyElectronicSubmissionReceiptRepository
    ) -> None:
    ...
    def test_find_idempotent_scoped_to_actor_device_operation(
        self, receipt_repo: SqlAlchemyElectronicSubmissionReceiptRepository
    ) -> None:
        r = ElectronicSubmissionReceipt(
            receipt_id="REC-002",
    ...
    def test_unique_constraint_on_idempotency(
        self, receipt_repo: SqlAlchemyElectronicSubmissionReceiptRepository
    ) -> None:
