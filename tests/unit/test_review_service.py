...
from app.domain.models import RecordStatus, ReviewStatus
...
def test_review_rejects_a_stale_expected_version(tmp_path: Path) -> None:
    imports, reviews, _ = build_services(tmp_path)
    image = tmp_path / "scan.png"
    image.write_bytes(b"image")
    imports.import_image(image, "FORM-0001", "T1", "1", "operator-a")
    reviews.confirm("FORM-0001", 0, {"total_quantity": 10}, "reviewer-a", "initial", ())

    with pytest.raises(ConcurrentReviewError):
        reviews.confirm("FORM-0001", 0, {"total_quantity": 11}, "reviewer-b", "stale", ())
