...
- [ ] 实现只读字典和 `getReviewStatusCopy`、`getExportStatusCopy`、`getTemplateStatusCopy`。
- [ ] 增加按钮文案常量，至少包含：上传表单照片、查找表单、开始审核、暂停审核、暂存修改、确认并审核下一张、保存本次修改、退回重新拍照、标记为无效、检查可导出的数据、生成 Excel。
- [ ] 再运行同一测试，预期通过。
- [ ] 暂不替换所有页面文案；本任务只建立单一来源。
- [ ] 提交：
...
- [ ] 先写 `task-api.test.ts`，用 mock fetch 验证 `TaskApi.getTask("TASK-1")` 请求 `/api/v1/tasks/TASK-1`，并解析：
...
- [ ] 增加失败响应测试：非 2xx 必须抛出 `ApiRequestError`，不能只显示 `Failed to fetch`。
- [ ] 运行测试，确认因客户端不存在而失败。
- [ ] 实现 `TaskApi`，沿用现有 API Client 的请求错误解析方式。
- [ ] 从 `index_ds.ts` 导出 `TaskApi` 和 `TaskStatusDetail`。
- [ ] 运行：
...
- [ ] 先在现有回归测试增加以下断言：
  ...
  - 无租约时写操作不可执行。
  - 有未保存修改时切换模块会出现确认提示。
- [ ] 运行测试并确认新增断言在当前代码上通过，形成拆分保护网。
- [ ] 把 `App.tsx` 中所有审核工作台状态、加载、上传、分类、租约、暂存、确认、退回、作废和渲染移动到 `ReviewWorkbenchPage.tsx`。
- [ ] `workbench-types.ts` 只放页面级类型：`MobilePane`、`QueueKey`、`ReviewAction`、`DuplicateImportInfo`。API 类型继续从 API Client 导入。
- [ ] `App.tsx` 暂时只保留四个功能区切换以及对 `ReviewWorkbenchPage`、模板、基础数据和导出组件的装配；本任务不改视觉、不加路由。
- [ ] 运行：
...
- [ ] 先写纯函数测试，定义优先级：填写错误 > 必填缺失 > 低可靠度 > 需要人工确认 > 正常字段。
- [ ] 测试 `selectFirstIssueFieldId(fields, edits, ruleFailures)`：加载表单后返回最高优先级问题字段；没有问题时返回第一个字段；空数组返回 `null`。
- [ ] 测试 `nextFieldId` 和 `previousFieldId` 在首尾边界不越界。
- [ ] 测试悬停字段与正式选中字段分离：清除 hover 不得改变 selected。
- [ ] 运行测试，确认失败；随后实现函数并通过。
- [ ] 所有判断复用 `reviewValueIssue`，不得创建不同的字段合法性规则。
- [ ] 提交：
...
- [ ] 先测试纯变换：缩放限制为 25%–400%，旋转每次 90°，复位返回 100%/0°/零位移，拖动只改变位移。
- [ ] 先写组件失败测试，覆盖：
  - 原图/校正图只在对应证据存在时可切换。
  - `+`、`-`、旋转、复位、显示/隐藏字段框可操作。
  - 鼠标滚轮缩放。
...
- [ ] 实现 `EvidenceToolbar` 和 `EvidenceViewer`。证据选择必须继续使用现有 `selectReviewEvidence` 语义。
- [ ] 字段框坐标使用现有 `source_region`，按实际图片显示尺寸换算；不得修改后端坐标。
- [ ] 增加选中字段裁片区域：优先显示 `FIELD_CROP` 且 `related_field_id` 匹配的证据；没有裁片时显示“该字段没有可用裁片”。
- [ ] 运行：
...
- [ ] 实现固定骨架：顶部表单上下文、左侧队列、中心双栏、下方详情、固定底部动作。
- [ ] 桌面分隔线可拖动，宽度限制为图片 35%–65%，把用户选择保存到 `localStorage` 键 `review-workbench-split-percent`。
- [ ] 窄屏使用“图片/电子表格”标签；切换时保留 `selectedFieldId` 和各自滚动位置。
- [ ] 字段详情显示裁片、候选值、填报规则、主数据选项和错误说明。
- [ ] 将状态文案全部改为任务 1 的业务字典；英文值仅放在追溯详情。
- [ ] 运行：
...
- [ ] 先写失败测试：二维码失败时显示原因、精确已发布版本选择、分类原因和模板摘要；未选版本或未填原因不能提交。
- [ ] 测试 `assignTemplate` 成功返回 `recognition_task_id` 后，不更换页面；每 500ms 查询一次任务状态。
- [ ] 状态为 `RUNNING` 时显示百分比与业务化步骤：校正图片、裁切字段、生成识别结果。
- [ ] 状态为 `SUCCEEDED` 时停止轮询，重新加载当前 workbench，并在同一双栏骨架中显示字段表。
- [ ] 状态为 `FAILED`、`CANCELLED` 或 `INTERRUPTED` 时停止轮询，说明发生了什么和下一步如何重试。
- [ ] 组件卸载或切换表单时使用 `AbortController` 或取消标记停止旧轮询；旧响应不得覆盖新表单。
- [ ] 实现并运行：
...
- [ ] 先测试快捷键：
  ...
  - `+`/`-`、`0`、`R` 只在图片查看器上下文生效。
- [ ] 测试按钮按状态隐藏：不适用动作不长期置灰；无表单时只显示上传和查找；待分类显示分类动作；待审核显示暂存/确认；已确认后修改显示保存本次修改；待重拍显示重拍说明。
- [ ] 实现三种专属空状态：待确认类型为空、待审核为空、待重新拍照为空。空状态不得渲染巨大空白双栏。
- [ ] 把通用网络错误转换为三段式提示：发生什么、为何不能继续、下一步怎么做。保留 `ApiRequestError.code` 到追溯详情。
- [ ] 退回和作废使用明确确认框并要求原因；保留现有 API 调用和审计语义。
- [ ] 运行工作台全部测试并提交：
...
- [ ] 先写测试：页面继续使用双栏骨架，显示上次照片、上次结果、退回原因、异常字段和历史记录入口。
- [ ] 页面把“规则异常”统一显示为“待重新拍照”。
- [ ] 主动作显示“上传新的表单照片”，但旁边必须明确说明：当前版本会创建一张新表单，尚不能自动关联为原表单的替换证据。
- [ ] 上传按钮可复用现有导入流程；成功后打开新表单。不得显示“已替换原照片”或“追溯关系已保存”。
- [ ] 在 `docs/NEXT_TASK.md` 的阻塞项保留独立后端需求：为原表单提交 replacement evidence、运行重新识别、保存 old/new evidence linkage 和审计事件。
- [ ] 运行测试并提交：
...
- [ ] 先用 `MemoryRouter` 写失败测试：一级导航为审核工作台、模板中心、基础数据、导出数据；当前项有 `aria-current="page"`；浏览器后退能回到上一位置。
- [ ] 建立以下路由，不能改名：
...
- [ ] `/` 重定向到 `/workbench/review`；未知地址显示应用内 404 和“返回审核工作台”。
- [ ] `AppShell` 固定显示产品名、一级导航、服务状态、当前身份位置和用户菜单占位；不得新增登录功能。
- [ ] 路由切换前复用现有未保存修改提醒；刷新后保留当前模块和表单 URL。
- [ ] 旧的 `Feature` 全屏替换状态删除；队列选择改由 URL 驱动。
- [ ] 运行：
...
- [ ] 先测试 `StatusBadge` 同时渲染文字和状态语义，不仅设置颜色。
- [ ] 在 `:root` 定义设计令牌：主操作蓝、成功绿、警告橙、危险红、次要灰；基础字号 14px，辅助文字不低于 12px；间距 8/16/24/32px；圆角 8/10px。
- [ ] 所有新增/保存/预览/下一步使用蓝色；绿色只用于成功或已启用状态；危险动作红色。
- [ ] 审核工作台尽量全宽；模板、基础数据、导出内容最大宽度 1520px。
- [ ] 减少卡片嵌套，优先使用标题栏、分隔线、浅色区、表格、抽屉和更多菜单。
- [ ] 在 1280px 以上显示双栏，在窄屏显示图片/电子表格标签，在 768px 以下保持主要动作可见且不横向溢出。
- [ ] 运行组件测试、类型检查和生产构建：
...
- [ ] 先测试模板库顶部工具栏含搜索、状态筛选、导入模板包、创建模板；“创建模板”打开抽屉或弹窗，不常驻左栏。
- [ ] 模板卡片中文名优先，编号放次级位置；只保留一个上下文主操作；退役、放弃草稿等危险动作放入“更多”。
- [ ] “模板包导入”继续禁用并显示“暂未开放”，不得假装已接通 API。
- [ ] 通过路由打开精确只读版本或草稿；发布版本只读，调优操作克隆新草稿。
- [ ] 预览页增加缩放、适合页面、适合宽度、复位、字段搜索、字段列表/画布双向联动、PDF/PNG 产物入口。
- [ ] 技术文件名、字段编号和哈希移入高级信息。
- [ ] 保持现有异步防串页测试：旧请求不得覆盖新选择。
- [ ] 运行：
...
- [ ] 先测试四个紧凑标签及正确路由：员工、工单、产品、工序；从产品 URL 打开时不得回到员工。
- [ ] 左侧列表桌面宽度固定约 340px，右侧为当前记录详情；新增按钮放在当前目录标题旁。
- [ ] 区分“目录为空”和“搜索无结果”的空状态。
- [ ] 用业务表单替代直接 JSON 编辑：
  - 员工：员工编号、姓名、班组、岗位、联系电话、备注。
  - 工单：工单编号、关联产品、计划数量、开始日期、结束日期、负责人。
  - 产品：产品编码、产品名称、规格型号、计量单位。
  - 工序：工序编码、工序名称、标准顺序、工作站。
- [ ] `master-data-form.ts` 负责业务字段与现有 attributes 对象双向转换；额外未知键进入“更多信息”键值编辑器，不得丢失。
- [ ] 保持稳定编码不可修改、If-Match 乐观版本、停用/恢复、审计和冲突提示语义。
- [ ] 冲突提示改为“该记录已被其他人修改，请刷新后重试”，技术版本放追溯详情。
- [ ] 运行：
...
- [ ] 先把现有测试扩展为四步可见流程：选择数据 → 检查数据 → 生成 Excel → 下载文件。
- [ ] 筛选区使用业务名称；主按钮为“检查可导出的数据”。
- [ ] 检查结果分为：将要导出的记录、无法导出的记录及原因、Excel 列对应关系。
- [ ] 只有有效预览存在时显示“生成 Excel”；筛选变化立即使预览和创建资格失效。
- [ ] 任务进行中显示业务进度；成功后在当前页面出现下载动作和历史记录。
- [ ] 默认隐藏完整批次 ID、SHA-256、映射哈希、内部文件名和技术状态码；在“追溯详情”抽屉中完整保留并提供复制。
- [ ] 重导文案说明：记录在上次导出后发生修改；选择包含旧数据的导出记录；系统生成修正版；旧文件继续保留。
- [ ] 保留现有所有并发保护：旧预览响应、旧刷新响应、卸载后的请求都不得覆盖新状态。
- [ ] 运行完整导出测试：
...
- [ ] 先测试 `ProblemNotice` 必须有标题、原因和下一步动作；原始 code 只在追溯详情显示。
- [ ] 先测试 `ConfirmDialog` 有可聚焦标题、说明、取消和明确危险动作；Esc 取消；关闭后焦点返回触发按钮。
- [ ] 统一退回、作废、模板停用、放弃草稿、基础数据停用的确认框，不再使用原生 `prompt`。
- [ ] 所有按钮使用“动作 + 对象”；图标按钮必须有 `aria-label`；导航、标签、表格、弹窗使用正确语义。
- [ ] 键盘完成审核主路径：进入字段、修改值、暂存、确认、关闭弹窗，焦点顺序可预测。
- [ ] 运行：
...
- [ ] 更新 `CURRENT_STATUS.md`：实际测试数、当前 HEAD、已完成阶段、仍阻塞的 replacement evidence API。
- [ ] 更新 `NEXT_TASK.md`：勾选已完成项，只保留真实未完成项；不要删除后端阻塞说明。
- [ ] 更新 `FRONTEND_INTERACTION_GUIDE.md` 为重建后的真实交互，不再保留已经失效的旧按钮说明。
- [ ] 检查文档没有占位词：
